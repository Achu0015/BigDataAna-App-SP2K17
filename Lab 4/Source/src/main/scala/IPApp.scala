...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("Digit_0", "Digit_1", "Digit_4", "Letter_A")
  ...
  def generateDecisionTreeModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(IPSettings.DECISION_TREE))) {
      println(s"${IPSettings.DECISION_TREE} exists, skipping Decision Tree model formation..")
      return
    }
          ...
          maxDepths.foreach(maxDepth => {
            maxBin.foreach(maxBins => {
              ...
              if (testErr < bestErr) {
                ...
                bestParams.put("maxBins", maxBins)
    ...
    //val randomForestModel = RandomForest.trainClassifier(parsedData, numClasses, categoricalFeaturesInfo, bestnumTrees, bestFeatureSubSet, bestimpurity, bestmaxdepth, bestmaxbin)
    val decisionTreeModel = DecisionTree.trainClassifier(parsedData, numClasses,categoricalFeaturesInfo, bestimpurity, bestmaxdepth,bestmaxbin)

    // Save and load model
    decisionTreeModel.save(sc, IPSettings.DECISION_TREE)
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val path = "files/101_ObjectCategories/ant/image_0012.png"
    ...
    val nbModel = DecisionTreeModel.load(sc, IPSettings.DECISION_TREE)
  ...
  def main(args: Array[String])  {
    ...
    val images = sc.wholeTextFiles(s"${IPSettings.INPUT_DIR}/*/*.png")
    ...
    val testImages = sc.wholeTextFiles(s"${IPSettings.TEST_INPUT_DIR}/*/*.png")
